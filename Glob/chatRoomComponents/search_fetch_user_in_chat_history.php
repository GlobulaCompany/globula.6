...
echo  '</tbody>
 ...
 ';
  ...
  <tbody><tr> <td width="100%" style="text-align:center; color:white;">No Chat History Available </td></tr>
        ...
        $query =mysqli_query($conn,"SELECT DISTINCT message_to_user_name, message_to_user_id,message_from_user_name,message_from_user_id,chat_message_id FROM chat_message  WHERE (message_from_user_id='$user_id' OR message_to_user_id='$user_id')  AND (message_to_user_name  LIKE '%$search%' OR message_from_user_name  LIKE '%$search%')  ORDER BY message_time DESC");
