...
$query_get = mysqli_query($conn,"SELECT * FROM video WHERE video_code = '{$video_code}'  ");
...
$user_id="";
while($get_user_id = mysqli_fetch_assoc($query_get)){
  $user_id=$get_user_id['user_id'];
   
  
}

$query = mysqli_query($conn,"SELECT * FROM video WHERE video_code != '{$video_code}' AND user_id='{$user_id}'   ORDER BY  RAND()");
...
<button   data-video_id="'.$result['video_id'].'" class="share-button text-light btn btn-success" ><span class="text-light  ">Share video</span></button>
 ...
 ';
  ...
  $output='<div  class="row">';
   ...
   $output .='   <video src="..." id=" "   width="100%" height="250px"></video>  ';

       $output .='
       <div class="container bg-success   d-flex justify-content-center">
       <button data-video_id=" " class="pause mr-2 m-1"  type="button"><i class="fa fa-stop-circle-o" style="color:red"></i></button>

       <button data-video_id=" " data-view_to_id=" " class="play mr-2 m-1 "  type="button"><i class="fa fa-play" style=" color:red"></i></button>
       <button data-video_id=" " class="pause m-1"  type="button"><i class="fa fa-pause" style="color:red"></i></button>

                  
          </div>

          <div class="card-header lead border border-info d-flex justify-content-center mt-2">
